...
gb = [Poly(1.0*s6**2 - 1.0*s6 - 2, s6, domain='RR'), Poly(1.0*s6 + 1.0*c1 + 1.0, s6, c1, domain='RR')]

# nmat = np.array([gb[0].coeffs(), gb[1].coeffs()])
# const = np.array([0, 0])


row1 = (gb[0].as_list())

# row1 = sp.lambdify(s6, gb[0])
row2 = (gb[1].as_list())
...
print("len(s6)=" + str(len(s6)))

myeval= [0]*len(s6)
for i in range(0, len(s6)):
  myeval[i] = np.polyval(row2, s6[i])


myeval = (np.flip(myeval))
